...
\section{方法}
\subsection{モデル化}
元信号$S$,混合行列$A$,混合信号$X$を用いて
\begin{gather}
    X = AS
\end{gather}
とする。このときある復元行列$B$を用いて復元信号$Y$を
\begin{gather}
    Y = BX
\end{gather}
のように書くことができるが、$Y$が現信号$S$に似た特徴をもったベクトルとなるような$B$を見つけられれば、信号がうまく分離できたと考えることができよう。ブラインド信号源分離問題はこのように行列$B$を推定する問題数式としてモデル化することができる。a

\subsection{ICA通信}
ICA通信ではこの問題を信号間の独立性を仮定することによって解くことを考える。
